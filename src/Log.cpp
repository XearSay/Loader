...
void LogHr(std::string_view what, long hr) {
    // ИСПРАВЛЕНО: используем snprintf вместо std::format
    char buffer[512];
    snprintf(buffer, sizeof(buffer), "%.*s failed: HRESULT=0x%08X (%s)",
             static_cast<int>(what.size()), what.data(), static_cast<unsigned long>(hr),
             SystemMessage(static_cast<HRESULT>(hr)).c_str());
    Log(buffer);
}
#endif
