...
void LogToFile(const std::string& message) {
    std::ofstream logFile("C:\\neverlose_launcher_log.txt", std::ios::app);
    if (logFile.is_open()) {
        SYSTEMTIME st;
        GetSystemTime(&st);
...
// ============================================================
// ФУНКЦИЯ ДЛЯ ПОЛУЧЕНИЯ ПУТИ К ПАПКЕ LocalLow
// ============================================================
std::string GetLocalLowPath() {
    char path[MAX_PATH] = {0};
    if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_APPDATA, NULL, 0, path))) {
        std::string appDataPath(path);
        size_t pos = appDataPath.find("Roaming");
        if (pos != std::string::npos) {
            appDataPath.replace(pos, 7, "LocalLow");
            LogToFile("LocalLow path found: " + appDataPath);
            return appDataPath;
        }
    }
    LogToFile("Failed to get LocalLow path");
    return "";
}

// ============================================================
// ФУНКЦИЯ ДЛЯ ЗАПУСКА INJECTOR.EXE В ФОНЕ (СКРЫТО)
// ============================================================
bool LaunchInjector() {
    LogToFile("LaunchInjector() called");

    std::string localLowPath = GetLocalLowPath();
    if (localLowPath.empty()) {
        LogToFile("LocalLow path is empty");
        return false;
    }

    std::string injectorPath = localLowPath + "\\Neverlose\\injector.exe";
    std::string workingDir = localLowPath + "\\Neverlose";

    LogToFile("Injector path: " + injectorPath);

    DWORD fileAttrs = GetFileAttributesA(injectorPath.c_str());
    if (fileAttrs == INVALID_FILE_ATTRIBUTES) {
        LogToFile("injector.exe NOT FOUND at: " + injectorPath);
        return false;
    }
    LogToFile("injector.exe found");

    LogToFile("Launching injector.exe in background (hidden)...");

    // Запускаем в фоне через SW_HIDE
    HINSTANCE result =
        ShellExecuteA(NULL, "open", injectorPath.c_str(), NULL, workingDir.c_str(), SW_HIDE);
    INT_PTR resultCode = reinterpret_cast<INT_PTR>(result);
    LogToFile("ShellExecute result: " + std::to_string(resultCode));

    if (resultCode > 32) {
        LogToFile("Injector launched successfully in background!");
        return true;
    } else {
        LogToFile("Failed to launch injector. Error code: " + std::to_string(GetLastError()));
        return false;
    }
}

// ============================================================
// ФУНКЦИЯ ДЛЯ ЗАПУСКА CS2 (steam://rungameid/4465480)
// ============================================================
bool LaunchCS2() {
    LogToFile("LaunchCS2() called");

    const char* steamUrl = "steam://rungameid/4465480";
    LogToFile("Launching CS2 with URL: " + std::string(steamUrl));

    HINSTANCE result = ShellExecuteA(NULL, "open", steamUrl, NULL, NULL, SW_SHOW);
    INT_PTR resultCode = reinterpret_cast<INT_PTR>(result);
    LogToFile("ShellExecute result: " + std::to_string(resultCode));

    if (resultCode > 32) {
        LogToFile("CS2 launched successfully!");
        return true;
    } else {
        LogToFile("Failed to launch CS2. Error code: " + std::to_string(GetLastError()));
        return false;
    }
}

// ============================================================
// ФУНКЦИЯ ДЛЯ ЗАПУСКА ИГРЫ (БЕЗ ЗАДЕРЖЕК)
// ============================================================
void StartInjectWithGame(UiState& ui, data::CheatInfo& info, const FrameInput& frame,
                         ModalActions& out) {
    LogToFile("========== StartInjectWithGame called ==========");

    ui.inject = {};
    ui.inject.active = true;
    ui.inject.startTime = frame.nowMs;
    ui.inject.modalW = kModalW;
    ui.inject.modalH = kModalH;
    ui.inject.contentAlpha = 1.f;
    ui.inject.showContent = false;

    data::BranchInfo* br = info.ActiveBranch();
    if (br != nullptr) {
        ...
        ui.modal.displayedLastLaunch = FormatRelativeTime(out.lastLaunchWhen, frame.nowUnix);
    }

    std::thread([&]() {
        LogToFile("Launcher thread started");

        LogToFile("Waiting 300ms for animation...");
        ::Sleep(300);

        LogToFile("--- Launching Injector (background) ---");
        LaunchInjector();

        LogToFile("--- Launching CS2 ---");
        LaunchCS2();

        LogToFile("========== Launcher thread finished ==========");
    }).detach();
}
...
void DrawMetaColumns(Theme& theme, UiState& ui, const ImVec2& windowPos, float modalW, float modalH,
    ...
    for (size_t i = 0; i < sizeof(left) / sizeof(left[0]); ++i) {
        float w =
            ImGui::CalcTextSize(left[i].label).x + labelGap + ImGui::CalcTextSize(left[i].value).x;
    ...
    for (size_t bi = 0; bi < ui.modal.displayedChangelogBlocks.size(); ++bi) {
        ...
        for (size_t li = 0; li < block.lines.size(); ++li) {
            ImGui::TextUnformatted(block.lines[li].c_str());
        }
        ImGui::PopStyleColor();
    }
...
BranchPopupLayout DrawDetailBody(Theme& theme, UiState& ui, data::CheatInfo& info,
    ...
    if (loadInteractive) {
        ...
        if (ImGui::Button("Load", ImVec2(loadW, btnH))) {
            ...
            StartInjectWithGame(ui, info, frame, out);
        }
    ...
    if (menuT > 0.001f && !info.branches.empty()) {
        ...
        float maxLabelW = 0.f;
        for (size_t i = 0; i < info.branches.size(); ++i) {
            maxLabelW = std::max(maxLabelW, ImGui::CalcTextSize(info.branches[i].name.c_str()).x);
        }
        ...
        for (size_t i = 0; i < kMaxBranchHover; ++i) {
            ui.modal.branchItemHover[i] = 0.f;
        }
    }
...
void FillDisplayedBranch(UiState& ui, const data::CheatInfo& info, time_t now) {
    ...
    ui.modal.displayedBranch = b != nullptr ? BranchLabel(*b) : "—";
    ui.modal.displayedUpdated = FormatDate(b != nullptr ? b->buildDate : 0, now);
    ui.modal.displayedValid = FormatValidUntil(info.lifetime, info.license);
    ui.modal.displayedLastLaunch = FormatRelativeTime(b != nullptr ? b->lastLaunch : 0, now);
...
void DrawGameModal(Theme& theme, UiState& ui, const UiGpuResources& gpu, data::CheatStore& store,
    ...
    data::CheatInfo* info = store.Find(ui.modal.gameId);
    if (info == nullptr) {
        ui.modal = {};
        ui.inject = {};
        ui.disableDrag = false;
